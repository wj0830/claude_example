...
- **Agent Access Boundary — 민감 파일은 에이전트 permission deny로 원천 차단한다.** `.claude/settings.json`의 `permissions.deny`에 `.env`(및 `.env.local`/`.env.production` 등 변형), `*.pem`/`*.key`/`*.pfx`/`*.p12`, `*secret*`/`*credential*`, SSH 개인키(`id_rsa*`, `id_ed25519*`), `.ssh/**`, `.aws/**`에 대한 Read/Edit/Write 차단 규칙이 등록되어 있다. `.env.example`은 실제 값이 없는 커밋된 템플릿이라 의도적으로 제외했다 — 새 env 변수 추가 시 편집이 필요하기 때문이다. 새로운 민감 파일 패턴(예: 새 클라우드 자격증명 파일)이 생기면 이 deny 목록에 추가하고, 반대로 실제 비밀값이 없는 템플릿/예시 파일을 deny에 넣지 않는다.
